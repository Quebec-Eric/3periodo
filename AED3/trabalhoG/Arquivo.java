...
import tabelaHex.HashExtensivel;
import tabelaHex.Registro;
...
public class Arquivo<T extends Registro> {
  ...
  public int creat(T ob) throws Exception {
    ...
    indiceDireto.create(new ParIDEndereco(ob.getID(), enderecoArk));
    ...
    if(indiceId!=null){
      ...
      if(lapida ==' '){
        ...
        if(registroAntigo.length>registroNovo.length){
          ...
          arquivo.write(registroNovo);
          //indiceDireto.update(new ParIDEndereco(obN.getID(), novoLugar,  Long.parseLong(obA.getEmail())));
          return true;
        }
